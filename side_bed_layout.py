...
from lib.shelf import ShelfUnitPly
...
class SideBedLayout(Item):
  ...
  def render(self):
    ...
    s.place( ShelfUnitPly((bw, shelf_depth, bed_shelf_h), count=1, desc="Above bed"), rel_to='TBR')
    ...
    s.place( ShelfUnitPly((bw, gl, bed_z), count=0, with_back=True, color=gc, desc="right garage"), rel_to='BR')
    s.place( ShelfUnitPly((sw-bw, gl, bed_z), count=2, with_back=True, color=gc, desc="left garage"), rel_to='BL')
    ...
    kc = (216, 230, 92)
    k_off = bl
    s.place( ShelfUnitPly((kw, bw, kh), count=1, color=kc, desc="kitchen unit"),
            rel_to='BR', rotation='R', offset=[0,k_off,0])
    s.place( ShelfUnitPly((bw, 4, sh-kh), count=3, color=kc, with_back=True, desc="spice rack"),
    ...
    s.place( ShelfUnitPly((long_shelf_w, shelf_depth, sh), count=3, desc="long side shelf"),
    ...
    s.place( ShelfUnitPly((gl, shelf_depth, ssh), count=3, desc="sideway back shelf"),
            rel_to='BL', rotation='L', offset=[0,0,bed_z])
    s.place( ShelfUnitPly((s_opt_w, shelf_depth, ssh), count=3, color=s_opt_c, desc="opt back shelf"),
